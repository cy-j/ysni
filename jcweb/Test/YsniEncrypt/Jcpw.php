...
* 使用方法:
*
* 1、将本段代码保存为 YsniEncrypt.php
*
* 2、在要加密的页面前面引入这个 php 文件
* require_once('YsniEncrypt.php');
*
* 3、设置页面访问密码
* YsniEncrypt('页面密码');
*
...
// 密码 Cookie 加密盐
if(!defined('YSNI_ENCRYPT_SALT'))
define('YSNI_ENCRYPT_SALT', 'Kgs$JC!V');
...
*/
function YsniEncrypt($password, $pageid = 'default') {
$pageid = md5($pageid);
$md5pw = md5(md5($password).YSNI_ENCRYPT_SALT);
$postpwd = isset($_POST['pagepwd']) ? addslashes(trim($_POST['pagepwd'])) : '';
$cookiepwd = isset($_COOKIE['YSNI_ENCRYPT_'.$pageid]) ? addslashes(trim($_COOKIE['YSNI_ENCRYPT_'.$pageid])) : '';

if($cookiepwd == $md5pw) return; // Cookie密码验证正确

if($postpwd == $password) { // 提交的密码正确
setcookie('YSNI_ENCRYPT_' . $pageid, $md5pw, time() + 3600000, '/');
return;
}
?>
<html>
